    ...
    try {
        ...
        if (id !== rows[0].usuario_id) {
            return res.status(401).json({
                mensagem: "Transação não pertence ao usuário logado.",
            });
        }
...
const deletarTransacao = async (req, res) => {
    ...
    try {
        ...
        await pool.query(`delete from transacoes where id = $1`, [
            transacao_id,
        ]);
...
const obterExtratoDeTransacoes = async (req, res) => {
    ...
    try {
        ...
        const filtrarEntrada = rows.filter(
            (transacao) => transacao.tipo === "entrada"
        );

        const filtrarSaida = rows.filter(
        ...
        const somaEntrada = entradas.reduce(
            (acumulador, valorAtual) => acumulador + valorAtual,
            valorInicial
        );

        const somaSaida = saidas.reduce(
            (acumulador, valorAtual) => acumulador + valorAtual,
            valorInicial
        );
